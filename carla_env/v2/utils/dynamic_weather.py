...
class Storm:
    ...
    def __str__(self) -> str:
        return f"Storm(clouds={self.clouds:.2f}%, rain={self.rain:.2f}%, wind={self.wind:.2f}%)"


class WeatherHandler:
    def __init__(self, world: carla.World, rng: np.random.Generator | None = None):
        self._world = world
        self._rng = rng if rng is not None else np.random.default_rng()
        self._dynamic = False

    def reset(self, cfg_weather: str) -> None:
        if hasattr(carla.WeatherParameters, cfg_weather):
            ...
            self._weather = self._rng.choice(WEATHERS)
