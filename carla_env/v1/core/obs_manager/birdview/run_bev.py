...
HOST_CARLA_SCRIPT = "/home/keishi_ishihara/opt/carla_0_9_15/CarlaUE4.sh"
...
def test_bev(client, tm):
    from carla_gym.core.obs_manager.birdview.chauffeurnet_wip import ObsManager
    ...
    for _ in range(80):
        route_plan.append((waypoint, 0))  # 0はダミーのtuple要素
        next_wp = waypoint.next(0.1)
        if not next_wp:
            break
        waypoint = next_wp[0]

    # 必要ならTrafficManagerの設定などもここで
    ...
    obs_manager = ObsManager(obs_configs)

    # parent_actorのモック（最低限必要な属性を持つオブジェクトを作る）
    class ParentActor:
        def __init__(self, vehicle):
            self.vehicle = vehicle
            self.route_plan = route_plan
            self.criteria_stop = type("Dummy", (), {"_target_stop_sign": None, "_stop_completed": True})()

    parent_actor = ParentActor(vehicle)
    obs_manager.attach_ego_vehicle(parent_actor)

    # 観測を取得
    obs = obs_manager.get_observation()

    # 結果を表示
    print("rendered shape:", obs["rendered"].shape)
    print("masks shape:", obs["masks"].shape)

    Image.fromarray(obs["rendered"]).save("rendered.png")
...
def single_gpu_demo():
    ...
    parser.add_argument("--rpc-port", type=int, default=2345)
