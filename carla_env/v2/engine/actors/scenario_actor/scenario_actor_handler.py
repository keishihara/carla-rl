...
class ScenarioActorHandler:
    ...
    def reset(self, task_config: dict, hero_vehicles: dict) -> None:
        ...
        for sa_id in actor_config:
            ...
            self.scenario_actors[sa_id] = EgoVehicle(carla_vehicle, target_transforms)
            # make agents
            module_str, class_str = actor_config[sa_id]["agent_entry_point"].split(":")
            AgentClass = getattr(
                import_module("carla_env.v2.engine.actors.scenario_actor.agents." + module_str), class_str
            )
            self.scenario_agents[sa_id] = AgentClass(
                self.scenario_actors[sa_id], self.hero_vehicles, **actor_config[sa_id].get("agent_kwargs", {})
            )
