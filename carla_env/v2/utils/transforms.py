...
def loc_global_to_ref(target_loc_in_global: carla.Location, ref_trans_in_global: carla.Transform) -> carla.Location:
    """Convert a location from global coordinate to reference coordinate.

    Args:
        target_loc_in_global: carla.Location in global coordinate (world, actor)
        ref_trans_in_global: carla.Transform in global coordinate (world, actor)

    Returns:
        carla.Location in ref coordinate
    """
...
def vec_global_to_ref(target_vec_in_global: carla.Vector3D, ref_rot_in_global: carla.Rotation) -> carla.Vector3D:
    """Convert a vector from global coordinate to reference coordinate.

    Args:
        target_vec_in_global: carla.Vector3D in global coordinate (world, actor)
        ref_rot_in_global: carla.Rotation in global coordinate (world, actor)

    Returns:
        carla.Vector3D in ref coordinate
    """
...
def rot_global_to_ref(target_rot_in_global: carla.Rotation, ref_rot_in_global: carla.Rotation) -> carla.Rotation:
    """Convert a rotation from global coordinate to reference coordinate.

    Args:
        target_rot_in_global: carla.Rotation in global coordinate (world, actor)
        ref_rot_in_global: carla.Rotation in global coordinate (world, actor)

    Returns:
        carla.Rotation in ref coordinate
    """
...
def rot_ref_to_global(target_rot_in_ref: carla.Rotation, ref_rot_in_global: carla.Rotation) -> carla.Rotation:
    """Convert a rotation from reference coordinate to global coordinate.

    Args:
        target_rot_in_ref: carla.Rotation in ref coordinate
        ref_rot_in_global: carla.Rotation in global coordinate (world, actor)

    Returns:
        carla.Rotation in global coordinate
    """
    ...
    target_rot_in_global = carla.Rotation(
        roll=target_roll_in_global, pitch=target_pitch_in_global, yaw=target_yaw_in_global
    )
...
def get_loc_rot_vel_in_ev(
    actor_list: List[carla.Actor], ev_transform: carla.Transform
) -> Tuple[List[List[float]], List[List[float]], List[List[float]]]:
    """Get location, rotation, and velocity of actors in ego vehicle coordinate.

    Args:
        actor_list: List of carla.Actor
        ev_transform: carla.Transform of ego vehicle

    Returns:
        Tuple of (location, rotation, absolute_velocity) in ego vehicle coordinate
    """
