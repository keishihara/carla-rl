    ...
    def get_observation(self, close_traffic_lights=None):
        ...
        for vehicle in vehicles:
            ...
            bounding_box = carla.BoundingBox(
                traffic_transform.location + vehicle.bounding_box.location, vehicle.bounding_box.extent
            )
        ...
        # road_mask, lane_mask
        road_mask = warp_affine(self._road, m_warp, (self._width, self._width), interpolation="bilinear").astype(bool)
        sidewalk_mask = warp_affine(
            self._sidewalk, m_warp, (self._width, self._width), interpolation="bilinear"
        ).astype(bool)
        lane_mask_all = warp_affine(
            self._lane_marking_all, m_warp, (self._width, self._width), interpolation="bilinear"
        ).astype(bool)
        lane_mask_broken = warp_affine(
            self._lane_marking_white_broken, m_warp, (self._width, self._width), interpolation="bilinear"
        ).astype(bool)
