...
def is_h5_map_exists(h5_file_path: Path, pixels_per_meter: float) -> bool:
    if not h5_file_path.exists():
        return False
    map_hf = h5py.File(h5_file_path, "r")
...
def write_h5_map(h5_file_path: Path, pixels_per_meter: float, dict_masks: dict):
    with h5py.File(h5_file_path, "w") as hf:
        ...
        hf.create_dataset(
            "lane_marking_all", data=dict_masks["lane_marking_all"], compression="gzip", compression_opts=9
        )
        hf.create_dataset(
...
def main():
    ...
    parser.add_argument("--save_dir", default="maps_{ppm}ppm")
    parser.add_argument("--pixels_per_meter", type=float, default=2.0)
    parser.add_argument("--carla_sh_path", default="~/opt/carla_0_9_15/CarlaUE4.sh")
    ...
    client_ports, carla_servers = [], []
    client_ports.append(current_port_0)
    carla_servers.append(
        subprocess.Popen(
            f"bash {args.carla_sh_path} -carla-rpc-port={current_port_0} -nosound -nullrhi "
    ...
    save_dir = script_dir / args.save_dir.format(ppm=args.pixels_per_meter)
    ...
    for carla_map in map_names:
        h5_file_path = save_dir / (carla_map + ".h5")
        if is_h5_map_exists(h5_file_path, pixels_per_meter):
            print(f"{str(h5_file_path.name)} with pixels_per_meter={pixels_per_meter:.2f} already exists.")
            continue

        print(f"Generating {h5_file_path.name} with pixels_per_meter={pixels_per_meter:.2f}.")
        world = client.load_world(carla_map, reset_settings=False)
        dict_masks = MapImage.draw_map_image(world.get_map(), pixels_per_meter)
        write_h5_map(h5_file_path, pixels_per_meter, dict_masks)
