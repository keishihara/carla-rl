...
from carla_env.v2.engine.observation.object_finder.pedestrian import ObsManager as OmPedestrian
from carla_env.v2.engine.observation.object_finder.vehicle import ObsManager as OmVehicle
from carla_env.v2.utils.hazard_actor import lbc_hazard_vehicle, lbc_hazard_walker
from carla_env.v2.utils.traffic_light import TrafficLightHandler
...
class ValeoAction:
    ...
    def get(self, terminal_reward):
        ...
        light_state, light_loc, _ = TrafficLightHandler.get_light_state(
            self._ego_vehicle.vehicle, offset=self._tl_offset, dist_threshold=18.0
        )
        ...
        angle_difference = np.deg2rad(
            np.abs(trans_utils.cast_angle(ev_transform.rotation.yaw - wp_transform.rotation.yaw))
        )
        ...
        reward_debug = {"debug_texts": debug_texts}
