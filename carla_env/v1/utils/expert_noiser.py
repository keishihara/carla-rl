...
class ExpertNoiser:
    ...
    def set_noise(self):
        if self.noise_type in {"Spike", "Throttle"}:
    ...
    def get_noise(self):
        if self.noise_type in {"Spike", "Throttle"}:
            if self.current_noise_mean > 0:
                return min(
                    0.55, self.current_noise_mean + (time.time() - self.noise_start_time) * 0.03 * self.intensity
                )
            else:
                return max(
                    -0.55, self.current_noise_mean - (time.time() - self.noise_start_time) * 0.03 * self.intensity
    ...
    def is_time_for_noise(self, steer):
        ...
        if (
            time.time() - self.noise_start_time >= self.noise_time_amount
            and not self.remove_noise
            and self.noise_being_set
        ):
            self.noise_being_set = False
    ...
    def compute_noise(self, action, speed):
        ...
        if self.noise_type == "Spike":
            if self.is_time_for_noise(action.steer):
                ...
                if self.remove_noise:
                    steer_noisy = max(min(steer + self.get_noise_removing() * (25 / (2.3 * speed + 5)), 1), -1)

                else:
                    steer_noisy = max(min(steer + self.get_noise() * (25 / (2.3 * speed + 5)), 1), -1)
