...
from carla_env.v2.engine.actors.ego.vehicle import EgoVehicle
...
class EgoVehicleHandler:
    ...
    def reset(self, task_config):
        ...
        for ev_id in actor_config:
            ...
            self.ego_vehicles[ev_id] = EgoVehicle(carla_vehicle, target_transforms, self._spawn_transforms, endless)

            self.reward_handlers[ev_id] = self._build_instance(self._reward_configs[ev_id], self.ego_vehicles[ev_id])
    ...
    def _build_instance(config, ego_vehicle):
        ...
        _Class = getattr(import_module("carla_env.v2.engine.actors.ego." + module_str), class_str)
    ...
    def tick(self, timestamp):
        reward_dict, terminated_dict, truncated_dict, info_dict = {}, {}, {}, {}
        ...
        for ev_id, ev in self.ego_vehicles.items():
            ...
            if timeout:
                terminated_dict[ev_id] = False
                truncated_dict[ev_id] = True
            else:
                terminated_dict[ev_id] = done
                truncated_dict[ev_id] = False
            ...
            episode_done = terminated_dict[ev_id] or truncated_dict[ev_id]
            if episode_done:
                ...
                outside_lane_length = (
                    np.sum([x["distance_traveled"] for x in self.info_buffers[ev_id]["outside_lane"]]) / 1000
                )
                wrong_lane_length = (
                    np.sum([x["distance_traveled"] for x in self.info_buffers[ev_id]["wrong_lane"]]) / 1000
                )

                if ev._endless:
                    score_route = completed_length
                elif info["route_completion"]["is_route_completed"]:
                    score_route = 1.0
                else:
                ...
                n_collisions = (
                    n_collisions_layout + n_collisions_vehicle + n_collisions_pedestrian + n_collisions_others
                )
        ...
        terminated_dict["__all__"] = all(terminated for obs_id, terminated in terminated_dict.items())
        truncated_dict["__all__"] = all(truncated for obs_id, truncated in truncated_dict.items())

        return reward_dict, terminated_dict, truncated_dict, info_dict
