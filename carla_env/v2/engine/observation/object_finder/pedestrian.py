...
import carla_env.v2.utils.transforms as trans_utils
from carla_env.v2.engine.observation.obs_manager import ObsManagerBase
...
class ObsManager(ObsManagerBase):
        ...
        self.obs_space = spaces.Dict(
            {
                ...
                "rotation": spaces.Box(low=-180, high=180, shape=(self._max_detection_number, 3), dtype=np.float32),
                "extent": spaces.Box(low=0, high=5, shape=(self._max_detection_number, 3), dtype=np.float32),
                "absolute_velocity": spaces.Box(
                    low=-5, high=5, shape=(self._max_detection_number, 3), dtype=np.float32
                ),
                "on_sidewalk": spaces.MultiBinary(self._max_detection_number),
                "road_id": spaces.Box(low=0, high=5000, shape=(self._max_detection_number, 1), dtype=np.int32),
                "lane_id": spaces.Box(low=-20, high=20, shape=(self._max_detection_number, 1), dtype=np.int32),
            }
        )
    ...
    def _debug_draw(self, pedestrian_list):
        ...
        for ped in pedestrian_list:
            self._world.debug.draw_point(ped.get_location(), color=carla.Color(b=255), life_time=0.1)
