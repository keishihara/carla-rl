...
def check_h5_maps(env_configs, obs_configs):
    ...
    save_dir = Path(hydra.utils.get_original_cwd()) / "carla_env/v1/core/obs_manager/birdview/maps"
    txt_command = (
        f"Please run map generation script from project root directory. \n"
        f"\033[93m"
        f"python -m carla_env.v1.utils.birdview_map "
...
def load_obs_configs(agent_configs_dict):
    ...
    for actor_id, cfg in agent_configs_dict.items():
        obs_configs[actor_id] = json.load(open(cfg["path_to_conf_file"]))["obs_configs"]
    return obs_configs
