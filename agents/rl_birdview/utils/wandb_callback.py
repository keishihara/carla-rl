...
class WandbCallback(BaseCallback):
    def __init__(self, cfg, vec_env):
        ...
        wandb.init(project=cfg.wb_project, notes=cfg.wb_notes, tags=cfg.wb_tags)
    ...
    def _on_training_end(self) -> None:
        print(f"[on_training_end] n_epoch: {self.n_epoch}, num_timesteps: {self.model.num_timesteps}")
        # save time
        ...
        if (self.model.num_timesteps - self._last_time_eval) >= self._eval_step:
            self._last_time_eval = self.model.num_timesteps
            # evaluate
            eval_video_path = (self._video_path / f"eval_step{self.model.num_timesteps}.mp4").as_posix()
    ...
    def _on_rollout_end(self):
        ...
        if (self.model.num_timesteps - self._last_time_buffer) >= self._buffer_step:
            self._last_time_buffer = self.model.num_timesteps
            buffer_video_path = (self._video_path / f"rollout_step{self.model.num_timesteps}.mp4").as_posix()

            list_buffer_im = self.model.buffer.render()

            clip = ImageSequenceClip(list_buffer_im, fps=30)
            clip.write_videofile(buffer_video_path, logger=None)
            wandb.log(
                {f"rollout/{self.model.num_timesteps}": wandb.Video(buffer_video_path)}, step=self.model.num_timesteps
            )
    ...
    def evaluate_policy(env, policy, video_path, min_eval_steps=1000):
        ...
        print(f"!!!!!!!!!!!! Evaluating policy... {min_eval_steps} steps")
        ...
        if len(list_render) > 0 and list_render[0] is not None:
            print(f"Saving video to {video_path}")
            clip = ImageSequenceClip(list_render, fps=30)
