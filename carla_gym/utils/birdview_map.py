    ...
    def get_lane_markings(lane_marking_type, lane_marking_color, waypoints, sign, pixels_per_meter, world_offset):
        ...
        marking_1 = [
            MapImage.world_to_pixel(
                MapImage.lateral_shift(w.transform, sign * w.lane_width * 0.5), pixels_per_meter, world_offset
            )
            for w in waypoints
        ]
...
if __name__ == "__main__":
    ...
    config = OmegaConf.load("config/train_envs/endless_dev.yaml")
    env_configs = []
    port = 2000
    for env_config in config:
        for gpu in env_config["gpu"]:
            single_env_cfg = OmegaConf.to_container(env_config)
            single_env_cfg["gpu"] = gpu
            single_env_cfg["port"] = port
            single_env_cfg["streaming_port"] = 0
    ...
    for env_config in env_configs:
        server_manager = CarlaServerManager(
            rpc_port=env_config["port"],
            streaming_port=env_config["streaming_port"],
            server_life_time_sec=60 * 60,
            use_apptainer=True,
        )
        server_manager.start(gpu_index=env_config["gpu"])
        server_managers.append(server_manager)
    ...
    for env_config in env_configs:
        carla_map = env_config["env_configs"]["carla_map"]
        ...
        print(sorted(client.get_available_maps()))
