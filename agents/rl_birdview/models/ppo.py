...
class PPO:
    ...
    def collect_rollouts(
        self, env: VecEnv, callback: BaseCallback, rollout_buffer: PpoBuffer, n_rollout_steps: int
    ) -> bool:
        ...
        while n_steps < n_rollout_steps:
            if n_steps % 100 == 0:
                steps_per_second = n_steps / (time.time() - rollout_start_time)
                elapsed = str(datetime.timedelta(seconds=time.time() - self.start_time)).split(".")[
                    0
                ]  # seconds to hh:mm:ss
                print(
    ...
    def learn(self, total_timesteps, callback=None, seed=2021):
        ...
        while self.num_timesteps < total_timesteps:
            ...
            print(
                f"[{num_trained=}] Training for {self.n_epochs} epochs w/ sample size {self.buffer.buffer_size * self.buffer.n_envs}"
            )
            t0 = time.time()
            self.train()
